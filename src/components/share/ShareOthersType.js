...
`;
const ShareOthersType = ({ history, vocabGroupData, onClickBack }) => {
  const onClickLook = useCallback(
    (group_code) => {
      history.push(`/share/others/look/${group_code}`);
    },
    [vocabGroupData],
  );
  ...
  const onClickMultiple = useCallback(
    (group_code) => {
      if(vocabGroupData.vocab_count >= 5){
        history.push(`/share/others/multiple/${group_code}`);
      }else{
  ...
  const onClickMultipleMean = useCallback(
    (group_code) => {
      if(vocabGroupData.vocab_count >= 5){
        history.push(`/share/others/multipleMean/${group_code}`);
      }else{
        alert('5단어 이상 그룹에 할당 후 이용 부탁드립니다.');
      }
    },
  ...
  const onClickSubjective = useCallback(
    (group_code) => {
      if(vocabGroupData.vocab_count >= 5){
        history.push(`/share/others/subjective/${group_code}`);
      }else{
        alert('5단어 이상 그룹에 할당 후 이용 부탁드립니다.');
      }
    },
  ...
  const onClickSubjectiveMean = useCallback(
    (group_code) => {
      if(vocabGroupData.vocab_count >= 5){
        history.push(`/share/others/subjectiveMean/${group_code}`);
      }else{
        alert('5단어 이상 그룹에 할당 후 이용 부탁드립니다.');
      }
    },
        ...
        <div className="row">
          <div className="col-12">
            ...
            <span className="vocab_count">
              ({vocabGroupData.vocab_count} 단어) by {vocabGroupData.nickname}
            </span>
          </div>
...
export default withRouter(ShareOthersType);
